...
# and 04; release tests enforce exact parity.
...
# Prior hyperparameters. These are identical to examples 03 and 04.
...
# MCMC. One script run is one chain; qsub runs scenario-chain jobs in parallel.
...
def main() -> None:
    ...
    for scenario in SCENARIOS:
        ...
        axis.set_title(f"{scenario['name']}: posterior latent predictor (Laplace-MH)")
        axis.legend()
        ...
        axis.set_title(
            f"{scenario['name']}: {PHASE_LABELS[FOCUS_PHASE - 1]} trajectory (Laplace-MH)"
        )
        axis.legend()
        for extension in FIGURE_FORMATS:
            figure.savefig(
        ...
        axis = predictive.plot(
            ...
            title=f"{scenario['name']}: posterior predictive check (Laplace-MH)",
        ...
        axis = forecast.plot(
            ...
            title=f"{scenario['name']}: posterior predictive forecast (Laplace-MH)",
            ylabel="temperature / °C",
        )
        figure = axis.figure
        for extension in FIGURE_FORMATS:
        ...
        axis = forecast.plot(
            ...
            history_dates=np.arange(N_TIME),
            history_points=FORECAST_HISTORY,
            title=f"{scenario['name']}: {PHASE_LABELS[FOCUS_PHASE - 1]} forecast (Laplace-MH)",
            ylabel="temperature / °C",
        )
        figure = axis.figure
        for extension in FIGURE_FORMATS:
        ...
        axis = forecast.plot(
            ...
            history_dates=np.arange(N_TIME),
            history_points=FORECAST_HISTORY,
            title=f"{scenario['name']}: seasonally adjusted level forecast (Laplace-MH)",
        ...
        axis.set_title(f"{scenario['name']}: posterior latent level (Laplace-MH)")
        for extension in FIGURE_FORMATS:
        ...
        axis.set_title(f"{scenario['name']}: posterior latent slope (Laplace-MH)")
        for extension in FIGURE_FORMATS:
        ...
        axis.set_title(f"{scenario['name']}: structural selection (Laplace-MH)")
        for extension in FIGURE_FORMATS:
            figure.savefig(figure_dir / f"selection.{extension}", dpi=FIGURE_DPI, bbox_inches="tight")
        plt.close(figure)

        figure, _ = laplace_fit.plot("process_sds", truths=truth["parameter_truth"], title=f"{scenario['name']}: prior to posterior (Laplace-MH)")
        for extension in FIGURE_FORMATS:
