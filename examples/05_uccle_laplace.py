...
# MCMC. One local run is one chain. For final inference, submit four
# independent chains and combine them; 1000 + 1000 is the release baseline.
...
def main() -> None:
    ...
    for number, name in enumerate(SERIES):
        ...
        axis.set_title(f"{name}: {focus_label} GEV-location trajectory (Laplace)")
        axis.set_ylabel("GEV location / °C")
        for extension in FIGURE_FORMATS:
            figure.savefig(
        ...
        axis = predictive.plot(
            ...
            title=f"{name}: posterior predictive check (Laplace)",
        ...
        axis = forecast.plot(
            ...
            title=f"{name}: posterior predictive forecast (Laplace)",
            ylabel="temperature / °C",
        )
        figure = axis.figure
        for extension in FIGURE_FORMATS:
        ...
        axis = forecast.plot(
            ...
            history_dates=values.index,
            history_points=FORECAST_HISTORY,
            title=f"{name}: {focus_label} posterior predictive forecast (Laplace)",
            ylabel="temperature / °C",
        )
        figure = axis.figure
        for extension in FIGURE_FORMATS:
        ...
        axis = forecast.plot(
            ...
            history_dates=values.index,
            history_points=FORECAST_HISTORY,
            title=f"{name}: seasonally adjusted level forecast (Laplace)",
        ...
        axis.set_title(f"{name}: posterior latent level (Laplace)")
        ...
        axis.set_title(f"{name}: structural selection (Laplace)")
        for extension in FIGURE_FORMATS:
