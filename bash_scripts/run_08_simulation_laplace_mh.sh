...
#   [MCMC_SEED] [RESULTS_ROOT] [RUN_ID] [OVERWRITE] [MH_STEPS]
...
echo "Running simulation study with Laplace-MH inference"
echo "n time      = ${N_TIME}"
echo "period      = ${PERIOD}"
...
for ((chain_index = 1; chain_index <= CHAINS; chain_index++)); do
  ...
  chain_log="logs/08_simulation_laplace_mh_${RUN_ID}_chain${chain_label}.log"
...
if (( failed != 0 )); then
  echo "At least one Laplace-MH chain failed; the combine step was not run." >&2
  exit 1
fi
...
for chain_run_id in "${chain_run_ids[@]}"; do
  matches=("${RESULTS_ROOT}/08_simulation_laplace_mh/${chain_run_id}__"*)
  if (( ${#matches[@]} != 1 )); then
