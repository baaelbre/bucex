...
for ((chain_index = 1; chain_index <= CHAINS; chain_index++)); do
  ...
  chain_log="logs/03_simulation_laplace_${RUN_ID}_chain${chain_label}.log"
...
if (( failed != 0 )); then
  echo "At least one Laplace chain failed; the combine step was not run." >&2
  exit 1
fi
...
for chain_run_id in "${chain_run_ids[@]}"; do
  matches=("${RESULTS_ROOT}/03_simulation_laplace/${chain_run_id}__"*)
  if (( ${#matches[@]} != 1 )); then
