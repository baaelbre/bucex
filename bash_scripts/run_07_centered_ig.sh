...
if (( CHAINS == 1 )); then
  ...
  "${PYTHON_BIN}" -u examples/07_centered_ig.py
...
for ((chain_index = 1; chain_index <= CHAINS; chain_index++)); do
  ...
  chain_log="logs/07_centered_ig_${RUN_ID}_chain${chain_label}.log"
...
for chain_run_id in "${chain_run_ids[@]}"; do
  matches=("${RESULTS_ROOT}/07_centered_ig/${chain_run_id}__"*)
  if (( ${#matches[@]} != 1 )); then
