...
| `observation/` | Gaussian/GEV likelihoods and seasonal scale declarations |
...
| `inference/fit/marginal.py` | Private FS/SSVS updates, seasonal scale, shape and residual correlation |
...
| `io/` | Checksummed schema 2.10 archives and historical readers |
...
`Model` remains the univariate entry point. A scalar `SeasonalScale` fit uses
one internal private channel and is returned as an ordinary scalar `FitResult`.
`MultiSeriesModel` with `MarginalPriors` uses private FS/exact-SSVS trajectories.
