...
def test_mixed_hierarchical_laplace_plan_is_explicitly_approximate():
    ...
    exact_plan = bx.plan(model, values, engine="laplace_mh")
    assert exact_plan.targets_exact_posterior
    assert exact_plan.approximation is None
...
def test_multiseries_progress_groups_shape_parameters_and_names_path_updates():
    line = mcmc_progress_line(
        ...
        metrics={
            "laplace_mh_acceptance": 0.75,
        },
    )
    assert "chain 2/4" in line
    assert "xi=(TXx:-0.12,TNn:0.04)" in line
    assert "laplace" in line.lower()


def test_laplace_screen_warm_starts_exact_laplace_mh_and_parallel_updates():
    ...
    assert np.all(
        np.isfinite(exact.draws_aux["laplace_mh_acceptance"])
    )
    assert np.all((exact.draws_aux["laplace_mh_acceptance"] >= 0)
                  & (exact.draws_aux["laplace_mh_acceptance"] <= 1))
...
def test_vectorized_gev_kernels_match_scalar_evaluation():
    ...
    vector = observation.logpdf(y_t, offset + states @ H, params=params_obs)
    scalar = np.asarray(
        [
            float(observation.logpdf(y_t, offset + state @ H, params=params_obs))
            for state in states
        ]
    )
